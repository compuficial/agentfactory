...
How `af` works, and why it's built the way it is. This is the maintained
design reference for contributors.
...
background `af` process to crash, upgrade, or babysit. Commands that
observe or act on sessions run a *reconciliation* pass that trues up the
database against tmux before answering; the dashboard does the same on
each tick. Store-only commands such as definition management do not need
that pass. You can delete the `af` binary, rebuild it, and pick up your
running agents exactly where they were.
...
Runtime substrate operations flow through the small `SessionBackend`
interface (create, attach preparation, capture, send, liveness, sizing,
and kill). Core and TUI code depend only on that interface. CLI
composition and `doctor` currently construct and inspect the tmux
implementation directly, so a second backend would require changes at
those composition/probe points, but not in lifecycle or dashboard logic.
...
3. Read newly appended log bytes once. Meaningful text is T1 activity;
   terminal protocol events are T1.75 state declarations.
4. Preserve explicit/hook `awaiting-input` and `done` until meaningful
   output. Terminal-declared state also yields to a newer protocol event;
   screen-detected state is re-evaluated on every quiet pass.
5. Apply a terminal verdict or meaningful output immediately.
6. Otherwise choose `idle`/`working` from `idle_threshold`; `starting`
   becomes `working` only on first output, or `idle` if the threshold
   elapses before any output.
7. For a quiet session, capture the rendered screen once and apply T1.5
   detection rules.
...
and `busy_timeout` serialize writes, while conditional updates compare
the status origin and log watermark observed at the start of the pass.
A stale reconciliation therefore cannot overwrite a newer explicit
signal or terminal transition.
...
| T1.5 | quiet rendered-screen patterns | `awaiting-input` |
| T1.75 | BEL, OSC notifications, OSC 133 marks | `awaiting-input`, `working` |
| T2 | hooks, scripts, or agents calling `af signal` | `awaiting-input`, `done` |

Higher tiers refine but never replace T0 liveness. Explicit T2 state is
sticky until meaningful output and outranks heuristic detection. `af`
injects `AF_SESSION_ID` / `AF_SESSION_NAME` into every session's
environment so hooks need zero per-session config.
...
- The data directory is created/hardened to `0700`; database, WAL/SHM,
  and log files are restricted to `0600`.
...
footer. It admits a bounded set of noninteractive commands and rejects
commands that block, replace the process, or serve a protocol. This is
why every command writes to `cmd.OutOrStdout()` instead of `os.Stdout`:
the admitted CLI and TUI paths share the same command implementations.
...
See [quality.md](quality.md) for the enforced gates and known gaps that
